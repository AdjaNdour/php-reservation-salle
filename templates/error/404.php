<div class="card" style="text-align: center; padding: 60px 20px; max-width: 600px; margin: 40px auto;">
    <div style="font-size: 4rem; margin-bottom: 16px;">🔍</div>
    <h1 style="color: #ef4444; font-size: 2rem; margin-bottom: 12px;">404 — Page introuvable</h1>
    <p style="color: var(--text-muted); font-size: 1.1rem; margin-bottom: 24px;">
        <?= htmlspecialchars($message ?? 'La ressource que vous recherchez semble introuvable ou a été déplacée.') ?>
    </p>
    <a href="/salles" class="btn btn-primary">Retour à l'accueil</a>
</div>
